...
![](https://alvin-jay.oss-cn-hangzhou.aliyuncs.com/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/rbtree/1.png)
...
```c
RB-INSERT-FIXUP（T,z） // z插入节点
 1 while color[p[z]] = RED  
 2     do if p[z] = left[p[p[z]]]  
 3           then y ← right[p[p[z]]]  // y叔叔节点
 4                if color[y] = RED  
...
15           else (if p[z] = right[p[p[z]]]) (same as then clause  
...
![](https://alvin-jay.oss-cn-hangzhou.aliyuncs.com/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/rbtree/8.png)
...
二叉查找树的删除代码如下所示：
```c
TREE-DELETE(T, z)  // z待删除节点
 1  if left[z] = NIL or right[z] = NIL
 2      then y ← z // y表示实际删除的节点
...
```c
 1 if left[z] = nil[T] or right[z] = nil[T]   // z待删除节点，y表示实际删除的节点
 2    then y ← z  
 3    else y ← TREE-SUCCESSOR(z)  
 4 if left[y] ≠ nil[T]  
 5    then x ← left[y]  
...
16 if color[y] = BLACK   // 如果被删除节点是黑色，则需要修复
17    then RB-DELETE-FIXUP(T, x)  // y表示实际删除的节点，可能为z；x为被删除节点y的替代者
18 return y  
```
...
![](https://alvin-jay.oss-cn-hangzhou.aliyuncs.com/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/rbtree/17.jpg)
