...
上一篇文章[Spring AOP源码分析(一)——筛选合适的通知器]()，分析了Spring是如何为目标bean筛选合适的通知器的。现在通知器选好了，接下来就要通过动态代理的方式将通知器(Advisor)所持有的通知(Advice)织入到bean的某些方法前后(即**创建代理对象**)。与筛选合适的通知器相比，创建代理对象的过程则要简单不少。下面先来了解一下相关的背景知识，再来分析创建代理对象的过程。
